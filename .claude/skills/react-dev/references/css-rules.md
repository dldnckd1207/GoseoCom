...
## 폰트 크기 체계 (`src/index.css` @theme inline)
...
프로젝트 전용 CSS 변수 (`src/index.css` `@theme inline`):
...
> **참고:** 아래 클래스 목록은 기준 가이드이며, 프로젝트 진행에 따라 실제 필요한 클래스를 추가한다. 2개 이상의 Page에서 동일한 Tailwind 조합이 반복되면 `layout.css`에 시맨틱 클래스로 등록한다.
...
> **`@apply` 사용 범위:** `shared/styles/layout.css` 및 슬라이스별 CSS 파일(`features/*/styles/*.css`, `pages/*/styles/*.css`, `entities/*/styles/*.css`)의 시맨틱 클래스 정의에 사용. 컴포넌트 내부에서는 Tailwind 유틸리티를 JSX에서 직접 사용.
...
| `pages/{slice}/styles/{slice}.css` | pages | 해당 page에서만 사용하는 전용 클래스 |
| `entities/{slice}/styles/{slice}.css` | entities | 해당 entity 도메인 전용 클래스 |

> **클래스 prefix 관례:** 슬라이스 CSS 파일 내 클래스명은 슬라이스명을 prefix로 사용한다 (예: `auth` feature → `.auth-*`, `home` page → `.home-*`). 소유권을 클래스명에서 즉시 식별 가능하게 한다.
...
/* 슬라이스 CSS (pages → features → entities 순서 권장) */
@import './{layer}/{slice}/styles/{slice}.css';
@import 'tw-animate-css';               /* 마지막: 애니메이션 유틸리티 */
```

> **규칙:** 모든 글로벌 CSS는 `src/index.css` 한 곳에서 명시적으로 import한다. 컴포넌트에서 직접 import 금지 (cascade 순서 보장).  
...
2. `src/index.css`에 import 한 줄 추가 — `shared/styles/layout.css` 다음, `tw-animate-css` 앞
...
| 요소 | 규칙 | 예시 |
|------|------|------|
| 이미지 | `alt` 속성 필수 | `<img alt="[이미지 내용 설명]" />` |
| 폼 요소 | `label` 연결 | `<label htmlFor="id">` + `<input id="id">` |
| 아이콘 버튼 | `aria-label` | `<button aria-label="[버튼 동작 설명]">` |
| 영역 | `aria-label` | `<section aria-label="[영역 역할 설명]">` |

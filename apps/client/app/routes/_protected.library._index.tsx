...
import type { Route } from './+types/_protected.library._index';
...
export async function loader({ request }: Route.LoaderArgs) {
    ...
    try {
        ...
        return { items: data.items, total: data.total, page: data.page, size: data.size, currentTab: tab, error: null };
    } catch (err) {
        if (err instanceof Response) throw err;
        return { items: [], total: 0, page: 1, size: 10, currentTab: tab, error: '번역 이력을 불러오지 못했습니다.' };
    }
}

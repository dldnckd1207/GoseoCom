...
def _to_summary(post: Post, board_code: str) -> PostSummaryResponse:
...
def _to_detail(post: Post, board_code: str, files: list[FileResponse]) -> PostDetailResponse:
...
class PostService:
    ...
    async def list_posts(
        ...
        posts, total = await self.repo.list(board_ids, req.keyword, req.page, req.size)
        return PageData(
            items=[_to_summary(p, board_map.get(p.board_id, "")) for p in posts],
            total=total,
            page=req.page,
            size=req.size,
        )
    ...
    async def get_post(self, post_id: str, payload: dict[str, Any]) -> PostDetailResponse:
        ...
        return _to_detail(post, board.board_code, files)
    ...
    async def create_post(
        ...
        return _to_detail(created, board.board_code, file_responses)
    ...
    async def update_post(
        ...
        return _to_detail(post, board.board_code, file_responses)
